...
3. Дальше идут последовательно (нумерация сверху вниз) json-строки с описанием локаций:
   3.1 Название локации ("name")
   3.2 Изображение, которое будет выводится на локации
      (когда будет графический интерфейс) ("image")
   3.3 Текст, который будет выводится на локации ("text")
   3.4 Переходы (в том же порядке, что и во втором пункте) ("choices"):
      3.4.1 Условия отображения выбора ("condition")
           (в формате python но без if и пока только с целыми числами)
      3.4.2 Текст выбора ("text")
      3.4.3 Изменение количества предметов в инвентаре ("gifts")
